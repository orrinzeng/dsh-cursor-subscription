...
function flattenBlocks(content) {
	...
	for (const block of content ?? []) {
		...
		else if (block.type === "tool-result") text += flattenBlocks(block.content);
	}
	return text;
}
...
function coldStartActionText(options) {
	...
	for (const message of options.messages ?? []) {
		...
		const text = message.role === "assistant" ? renderAssistant(message.content) : flattenBlocks(message.content).trim();
...
/**
 * DSH `LlmAdapter` for the Cursor subscription. Each `stream()` call opens
 * one agent run with the full conversation history rebuilt from DSH messages;
 * when the model requests an MCP tool the run ends with a `tool-calls`
 * finish and DSH executes the tool, then the next call carries the result
 * back as history.
 */
export class CursorAdapter extends LlmAdapter {
	...
	async *stream(options) {
		...
		let run;
		// Persisted conversation state per session: the server's checkpoint and
		// the blobs it references (captured from setBlobArgs / checkpoint frames).
		...
		// Work on a copy so a failed/aborted run cannot corrupt the last known-good
		// checkpoint or its blob store.
		const persisted = {
		...
		try {
			...
			const { payload, blobStore } = buildRunPayload(options, options.model, persisted);
			...
			await run.start();
			const abortRun = () => run.abort(new Error("Cursor request aborted by caller"));
			upstream.addEventListener("abort", abortRun, { once: true });

			run.writeMessage(payload);
			run.startHeartbeat();
				...
				for (;;) {
					...
					if (message.case === "conversationCheckpointUpdate") {
						...
						if (exec.case === "requestContextArgs") {
							...
							// End the run: DSH executes the tool and calls us again.
							run.close();
							streamClosed = true;
							break;
						} else {
			...
			if (sessionKey !== undefined && persisted.checkpoint !== undefined) {
